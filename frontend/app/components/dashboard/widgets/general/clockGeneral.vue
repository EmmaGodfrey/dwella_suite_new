...
function clock() {
  const cd = new Date();
  clockset.value.hour = 30 * ((cd.getHours() % 12) + cd.getMinutes() / 60);
  clockset.value.min = 6 * cd.getMinutes();
  clockset.value.sec = 6 * cd.getSeconds();
  clockset.value.date = cd.getDate();
  clockset.value.month = months.value[cd.getMonth()]!;
  clockset.value.year = cd.getFullYear();
  if (document.getElementById('sec') !== null && document.getElementById('min') !== null && document.getElementById('hour') !== null) {
    document.getElementById('sec')!.style.cssText = '-webkit-transform:rotate(' + clockset.value.sec + 'deg);';
    document.getElementById('min')!.style.cssText = '-webkit-transform:rotate(' + clockset.value.min + 'deg);';
    document.getElementById('hour')!.style.cssText = '-webkit-transform:rotate(' + clockset.value.hour + 'deg);';
  }
  setTimeout(clock, 10);
}
</script>
